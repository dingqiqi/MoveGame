package com.dingqiqi.testmove9image.activity;
...
import com.dingqiqi.testmove9image.util.FileUtil;
import com.dingqiqi.testmove9image.util.ImageUtil;
import com.dingqiqi.testmove9image.view.MoveView;
import com.dingqiqi.testmove9image.R;
import com.dingqiqi.testmove9image.dialog.SelectImageDialog;
import com.dingqiqi.testmove9image.dialog.SelectLevelDialog;
import com.dingqiqi.testmove9image.dialog.ShowImageDialog;
    ...
    private void initView() {
        mMoveView.setOnCallBack(new MoveView.onCallBack() {
            ...
            public void onSuccess() {
                Toast.makeText(MainActivity.this, "拼图成功!", Toast.LENGTH_SHORT).show();
            }
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == SelectImageDialog.SELECT_IMAGE) {
                ...
                if (mIsZoom) {
                    ...
                    // 拿到图片路径
                    String path = FileUtil.getImagePath(MainActivity.this, mImageUri);
                ...
                if (mCurPhotoFile.exists()) {
                    //是否裁剪
                    if (mIsZoom) {
                        Uri mImageUri = FileProvider.getUriForFile(MainActivity.this, getPackageName() + ".FileProvide", mCurPhotoFile);
                        mImageDialog.startPhotoZoom(mImageUri);
                    } else {
                ...
                // 拿到剪切数据
                Bitmap bitmap = FileUtil.getImageBitmap(MainActivity.this, data);
                if (bitmap != null) {
                    setGameBg(bitmap);
                } else {
